import numpy as np

# ===============================================
# 1. PARÁMETROS GEOMÉTRICOS DEL ESCENARIO
# ===============================================
# Dimensiones de la Fábrica (m)
L = 7.0   # Largo
W = 7.0   # Ancho
H = 3.0   # Alto

# Altura del Receptor (Plano de trabajo del Cobot)
H_RX = 0.85 

# Coordenadas de los 4 LEDs (Transmisores) en el techo (MIMO 4x1)
# Formato: [x, y, z] para cada LED
LED_COORDS = np.array([
    [1.75, 1.75, H],
    [1.75, 5.25, H],
    [5.25, 1.75, H],
    [5.25, 5.25, H]
])

# ===============================================
# 2. PARÁMETROS ÓPTICOS Y DEL TRANSMISOR
# ===============================================
P_TX = 0.5                      # Potencia Óptica Transmitida por cada LED (W)
RESPONSIVITY = 0.5              # Responsividad del Fotodiodo (R) (A/W)
A_RX = 1.0e-4                   # Área Física del Fotodiodo (m^2)

# Parámetros Lambertianos (Modelo del Haz)
PHI_HALF = np.deg2rad(60)       # Ángulo de semi-potencia del LED (60 grados)
# Calculamos el orden m una sola vez aquí para no repetir el cálculo en el bucle
M_ORDER = -np.log(2) / np.log(np.cos(PHI_HALF)) 

# Parámetros del Receptor
FOV_MAX = np.deg2rad(60)        # Campo de Visión máximo (FOV) del receptor (radianes)

# Parámetros para NLOS Simplificado (Modelo de esfera)
RHO_REFLECTION_1 = 0.8          # Reflectividad de primer rebote
RHO_AVG = 0.7                   # Reflectividad ponderada promedio
A_ROOM = 2 * (L*W + L*H + W*H)  # Área total de la superficie de la sala para reflexiones

# ===============================================
# 3. PARÁMETROS DE RUIDO Y SIMULACIÓN
# ===============================================
B = 100e6                       # Ancho de banda del sistema (Hz)
R_LOAD = 50                     # Resistencia de carga (Ohmios)
I_BG = 10e-6                    # Corriente de fondo por luz ambiental (A)

# Constantes Físicas
Q_ELECTRON = 1.602e-19          # Carga del electrón (C)
K_BOLTZMANN = 1.38e-23          # Constante de Boltzmann (J/K)
T_AMBIENTE = 295.15             # Temperatura ambiente BASE (22°C en Kelvin)
T_EXTREMA = 310.15              # Para probar condiciones extremas (37°C)

# Resolución de la simulación
GRID_SIZE = 50                  # La cuadrícula será de 50x50 puntos (2500 iteraciones)

# ===============================================
# 4. VARIABLES DEL ENTORNO HOSTIL (VARIABLES DE PRUEBA)
# ===============================================

# 4.1. Atenuación por Polvo (Ley de Beer-Lambert)
COEF_EXTINCION_ALPHA = 0.1      

# 4.2. Bloqueo por Obstáculo (Shadowing)
OBSTACLE_PRESENTE = True        # True para activar el obstáculo
OBSTACLE_X = 3.0                # Coordenada X donde empieza
OBSTACLE_Y = 3.0                # Coordenada Y donde empieza
OBSTACLE_WIDTH = 1.0            # <--- ¡ESTA ES LA VARIABLE QUE TE FALTA!
OBSTACLE_DEPTH = 1.0            # Profundidad (m)
OBSTACLE_HEIGHT = 2.0           # Altura (m)